...
static uint8 rx_ask_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0x02, 0, 0};
...
#define ASK_RX_TO_POLL_TX_DLY_UUS	2600
...
int main(void)
{
  ...
  //dwt_setpreambledetecttimeout(PRE_TIMEOUT);		//设置前导码超时
		...
		if (status_reg & SYS_STATUS_RXFCG)
    {
			...
			// 是否为ASK消息
			if (memcmp(rx_buffer, rx_ask_msg, ALL_MSG_COMMON_LEN) == 0)
      {
				// 发送Poll消息
        ...
        if( dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED) == DWT_ERROR)		//发送后并开启接收器
				{
					printf("poll error");
				}
